	...
	def get(self,request,*args,**kwargs):
		...
		if request.method == 'GET':
			...
			for fi in codigos:
				...
				if citas.objects.filter(fichas_id=fi.id).exists():
					if citas.objects.filter(fichas_id=fi.id,autorizacion=0):
						autorizacion =0
					else:
						autorizacion =1
				...
				if citas.objects.filter(fichas_id=fi.id).exists():
					if autorizacion==1:
						...
						data = {
							    'status': '401', 'reason': 'No se han aprobado todas las Citas'  
							}
						return HttpResponse( json.dumps( data), content_type='application/json', status=401)		
				else:
					data = {
						    'status': '401', 'reason': 'No existen Citas'  
						}
					return HttpResponse( json.dumps( data), content_type='application/json', status=401)
