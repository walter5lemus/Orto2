...
urlpatterns = [
 ...
 url(r'^editar2/(?P<codi>[0-9]{4}(.).*?((?:[a-z][a-z0-9_]*)?))/(?P<num>\d+)/(?P<numeroCita>\d+)/$', login_required(citas_editar2), name= 'citaseditar2'),
 ...
 url(r'^eliminar/', login_required(eliminar_cita), name= 'eliminar_cita'),
